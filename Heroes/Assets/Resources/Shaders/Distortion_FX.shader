...
Properties {
	...
	[Enum(Off,0,On,1)] _fTAE("Animation <1>", Float) = 0.0			//togle firstTextureAnimationEnab
	...
	_Scale2("Scale(XY) <1>, PivotXY(ZW))", Vector) = (1,1,0.5,0.5)
	...
	_Scale3("Scale(XY) <1>, PivotXY(ZW))", Vector) = (1,1,0.5,0.5)
	...
	SubShader {
		Pass {
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				if(_numberTex > 0.5)
				{
					i.texcoord2 = Scale(i.texcoord,_Scale2);
					if(_sTAE > 0.5)				
				...
				if(_numberTex > 1.5)
				{
					i.texcoord3 = Scale(i.texcoord,_Scale3);
					if(_tTAE > 0.5)				
